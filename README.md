...
Para isso, é necessário realizar a chamada ao método de mesmo nome **TOKEN**, na qual é necessário o invocar duas vezes para obter todos os parâmetros necessários para o fluxo para obter os produtos disponibilizados aos cliente e o ID de autorização. Desta forma, segue abaixo a descrição dos passos a serem realizados.
- 1º Ao realizar a chamada sem o parâmetro *storeId* (storeId = null) será retornado uma lista de stores (StoreID - TABs) cadastrados para o cliente, identificado pelas credenciais de autenticação
- 2º Ao informar o storeId (Code - TAB) é retornado o token de autenticação de sessão [BEARER] para a confirmação do fluxo para o financiamento.
